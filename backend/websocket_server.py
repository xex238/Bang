...
class Server:
    ...
    rooms_ports = [] # Список портов для игровых онлайн комнат
    rooms_status = [] # Статус портов для игровых онлайн комнат
    rooms_server = [] # Серверы комнат
    rooms_ID = [] # Список с ID комнаты для каждого порта
    ...
    def __init__(self):
        for i in range(self.count_of_rooms):
            ...
            self.rooms_server.append(websockets.serve(self.room, "localhost", self.rooms_ports[i]))
            self.rooms_ID.append(-1)
    ...
    async def get_room_port(self, websocket, path):
        try:
            ...
            room_port = - 1
            for i in range(self.count_of_rooms):
                if(self.rooms_ID[i] == ID):
                    room_port = self.rooms_ports[i]
    ...
    # Функционирование комнаты. Обмен данными между её участниками
    async def room(self, websocket, path):
        try:
            self.conn = pyodbc.connect(str(self.driver + self.server + self.database + self.user + self.password + self.TC))
